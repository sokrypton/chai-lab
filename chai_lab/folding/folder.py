# -*- coding: utf-8 -*-
"""
ChaiFolder — modular wrapper around Chai-1 for protein structure prediction.
"""
...
from chai_lab.chai1 import _bin_centers, DiffusionConfig, load_exported, _get_distogram_head
...
class FoldingState:
    ...
    distogram_logits: Optional[Tensor] = None  # [n_tokens, n_tokens, n_dist_bins]
    ...
    def copy(self) -> "FoldingState":
        ...
        return FoldingState(
            ...
            distogram_logits=self.distogram_logits.clone() if self.distogram_logits is not None else None,
        )
...
class ChaiFolder:
    ...
    def save_state(self) -> FoldingState:
        ...
        saved_state = FoldingState(
            ...
            distogram_logits=self.state.distogram_logits.clone() if self.state.distogram_logits is not None else None,
        )
        return saved_state
    ...
    def _build_structure_context(
        ...
        restraints: Optional[List[PairwiseInteraction]] = None,
    ) -> AllAtomStructureContext:
        """Build merged structure context with optional pseudo_monomer and covalent bonds."""
        ...
        # Apply covalent bond restraints to the structure context
        if restraints:
            cov_a, cov_b = get_atom_covalent_bond_pairs_from_constraints(
                provided_constraints=restraints,
                token_residue_index=structure_context.token_residue_index,
                token_residue_name=structure_context.token_residue_name,
                token_subchain_id=structure_context.subchain_id,
                token_asym_id=structure_context.token_asym_id,
                atom_token_index=structure_context.atom_token_index,
                atom_ref_name=structure_context.atom_ref_name,
            )
            if cov_a.numel() > 0 and cov_b.numel() > 0:
                orig_a, orig_b = structure_context.atom_covalent_bond_indices
                if orig_a.numel() == orig_b.numel() == 0:
                    structure_context.atom_covalent_bond_indices = (cov_a, cov_b)
                else:
                    structure_context.atom_covalent_bond_indices = (
                        torch.cat([orig_a, cov_a]),
                        torch.cat([orig_b, cov_b]),
                    )
    ...
    def _build_feature_contexts(
        ...
        if restraints:
            restraint_context = load_manual_restraints_for_chai1(
                chains, crop_idces=None, provided_constraints=restraints
            )
        else:
    ...
    def prep_inputs(
        self,
        sequences: List[Tuple[str, str, str, Dict]],
        restraints: Optional[List[PairwiseInteraction]] = None,
    ) -> None:
        """Prepare inputs for folding.

        Args:
            sequences: List of [sequence, chain_name, entity_type, options_dict].
                chain_name (e.g. "A", "B") is also used as the subchain identifier
                in restraints.
            restraints: Optional list of PairwiseInteraction restraints.
                Supported types:
                  - PairwiseInteractionType.CONTACT  — soft distance restraint
                  - PairwiseInteractionType.POCKET   — binding-site restraint
                  - PairwiseInteractionType.COVALENT — covalent bond between atoms
                chainA / chainB in each restraint must match the chain_name values
                passed in sequences (e.g. "A", "B").
        """
        ...
        with torch.no_grad():
            ...
            # Build contexts
            structure_context = self._build_structure_context(chains, opts_per_chain, restraints)
            ...
            self.state.feature_context = self._build_feature_contexts(
                chains, structure_context, opts_per_chain, restraints
            )
    ...
    def run_trunk(self, num_trunk_recycles: int = 3, template_weight = None) -> None:
        ...
        if self._init_reps is None:
             raise RuntimeError("Must call get_embeddings() before run_trunk()")
        ...
        distogram_head = _get_distogram_head(self.device)
        if distogram_head is not None:
            with torch.no_grad():
                disto_logits = distogram_head.compute_disto_logits(s_pair.float())
                # Symmetrize: [1, N_padded, N_padded, n_dist_bins]
                disto_logits = (disto_logits + disto_logits.transpose(1, 2)) / 2
                token_mask = inputs_gpu["token_exists_mask"].squeeze(0)
                # Crop to valid tokens and move to CPU: [n_tokens, n_tokens, n_dist_bins]
                self.state.distogram_logits = disto_logits.squeeze(0)[token_mask][:, token_mask].cpu()
    ...
    def sample(self,
        ...
        if self._trunk_reps is None:
             raise RuntimeError("Must call run_trunk() before sample()")
    ...
    def get_distogram(self, as_probs: bool = True):
        """
        Return distogram output from the last run_trunk() call.

        Args:
            as_probs: If True, return softmax probabilities [n_tokens, n_tokens, n_dist_bins].
                      If False, return raw logits.

        Returns:
            Tensor [n_tokens, n_tokens, n_dist_bins] on CPU, or None if unavailable.
        """
        if self.state.distogram_logits is None:
            return None
        if as_probs:
            return self.state.distogram_logits.float().softmax(dim=-1)
        return self.state.distogram_logits
